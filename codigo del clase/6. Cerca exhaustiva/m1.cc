...
pair<int,vector<bool>> knapsack (int n, int W, const vector<int>& pes, const vector<int>& valor,
  ...
  if (idx == n) {
    ...
    sol[idx] = false;
    pair<int,vector<bool>> s1 = knapsack(n,W,pes,valor,sol,idx+1);

    sol[idx] = true;
    pair<int,vector<bool>> s2 = knapsack(n,W,pes,valor,sol,idx+1);
...
pair<int,vector<bool>> knapsack (int n, int W, const vector<int>& pes, const vector<int>& valor) {
  vector<bool> sol(n);
  int idx = 0;
  return knapsack(n,W,pes,valor,sol,idx);
}
